...
ManualParser::ManualParser(const std::vector<Token>& token_list) 
    : tokens(token_list), current_token_index(0), has_error(false) {}

const Token& ManualParser::current_token() {
...
const Token& ManualParser::peek_token(size_t offset) {
...
void ManualParser::advance_token() {
    if (current_token_index < tokens.size()) {
        current_token_index++;
    }
}

bool ManualParser::match(TokenType type) {
    return current_token().type == type;
}

bool ManualParser::consume(TokenType type, const std::string& error_msg) {
    if (match(type)) {
        advance_token();
        return true;
    } else {
...
void ManualParser::report_error(const std::string& message) {
    has_error = true;
...
std::unique_ptr<CompUnit> ManualParser::parse() {
    return parse_comp_unit();
}

std::unique_ptr<CompUnit> ManualParser::parse_comp_unit() {
...
std::unique_ptr<FuncDef> ManualParser::parse_func_def() {
...
std::vector<std::string> ManualParser::parse_param_list() {
...
std::unique_ptr<Block> ManualParser::parse_block() {
...
std::unique_ptr<Stmt> ManualParser::parse_stmt() {
    // Block
...
std::unique_ptr<Expr> ManualParser::parse_expr() {
    return parse_lor_expr();
}

std::unique_ptr<Expr> ManualParser::parse_lor_expr() {
...
std::unique_ptr<Expr> ManualParser::parse_land_expr() {
...
std::unique_ptr<Expr> ManualParser::parse_rel_expr() {
...
std::unique_ptr<Expr> ManualParser::parse_add_expr() {
...
std::unique_ptr<Expr> ManualParser::parse_mul_expr() {
...
std::unique_ptr<Expr> ManualParser::parse_unary_expr() {
...
std::unique_ptr<Expr> ManualParser::parse_primary_expr() {
    // 数字
...
std::vector<std::unique_ptr<Expr>> ManualParser::parse_arg_list() {
...
BinaryExpr::Operator ManualParser::token_to_binary_op(TokenType type) {
    switch (type) {
...
UnaryExpr::Operator ManualParser::token_to_unary_op(TokenType type) {
    switch (type) {
