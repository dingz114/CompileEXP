...
void IRGenerator::optimize() {
    ...
    constantPropagationCFG();       // 常量传播
    copyPropagationCFG();           // 复制传播
    
    // 循环优化
    loopInvariantCodeMotion();      // 新增：循环不变量外提
    
    // 函数优化
    functionInlining();             // 新增：函数内联
 ...
 * 深度克隆操作数
 */
std::shared_ptr<Operand> cloneOperand(const std::shared_ptr<Operand>& op) {
    if (!op) return nullptr;
    auto cloned = std::make_shared<Operand>(op->type, op->name);
    cloned->value = op->value;
    return cloned;
}

std::shared_ptr<IRInstr> IRGenerator::cloneInstruction(const std::shared_ptr<IRInstr>& instr) {
    // 这里只实现几种常见指令的克隆
    if (auto binOp = std::dynamic_pointer_cast<BinaryOpInstr>(instr)) {
        return std::make_shared<BinaryOpInstr>(
            binOp->opcode,
            cloneOperand(binOp->result),
            cloneOperand(binOp->left),
            cloneOperand(binOp->right));
    }
    else if (auto unaryOp = std::dynamic_pointer_cast<UnaryOpInstr>(instr)) {
        return std::make_shared<UnaryOpInstr>(
            unaryOp->opcode,
            cloneOperand(unaryOp->result),
            cloneOperand(unaryOp->operand));
    }
    else if (auto assign = std::dynamic_pointer_cast<AssignInstr>(instr)) {
        return std::make_shared<AssignInstr>(
            cloneOperand(assign->target),
            cloneOperand(assign->source));
    }
...
void IRGenerator::replaceParameters(std::shared_ptr<IRInstr>& instr,
                                  const std::unordered_map<std::string, std::shared_ptr<Operand>>& paramMap) {
    // 简化实现：暂时不做参数替换
}

void IRGenerator::renameTemporaries(std::shared_ptr<IRInstr>& instr,
                                  std::unordered_map<std::string, std::shared_ptr<Operand>>& tempMap) {
    // 简化实现：暂时不重命名临时变量
}

// IRAnalyzer 辅助函数实现
bool IRAnalyzer::hasSideEffects(const std::shared_ptr<IRInstr>& instr) {
    return std::dynamic_pointer_cast<CallInstr>(instr) != nullptr ||
           std::dynamic_pointer_cast<ReturnInstr>(instr) != nullptr ||
           std::dynamic_pointer_cast<GotoInstr>(instr) != nullptr ||
           std::dynamic_pointer_cast<IfGotoInstr>(instr) != nullptr;
}

bool IRAnalyzer::isPureFunction(const std::vector<std::shared_ptr<IRInstr>>& instructions) {
...
int IRAnalyzer::countInstructions(const std::vector<std::shared_ptr<IRInstr>>& instructions) {
    int count = 0;
    for (auto& instr : instructions) {
...
void IRGenerator::loopInvariantCodeMotion() {
                    ...
                    if (pred == block) {
                        ...
                        while (!worklist.empty()) {
                            ...
                            for (auto next : current->successors) {
                                if (next != block && loop.blocks.find(next->id) == loop.blocks.end()) {
                                    loop.blocks.insert(next->id);
                                    worklist.push(next);
                                }
                            }
                        }
                        ...
                        if (loop.blocks.size() > 1) {
                            // 创建preheader
                            auto preheader = std::make_shared<BasicBlock>();
                            preheader->id = blocks.size() + detectedLoops.size();
                            preheader->label = "__preheader_" + std::to_string(detectedLoops.size());
                            preheader->functionName = block->functionName;
                            preheader->instructions.push_back(
                                std::make_shared<LabelInstr>(preheader->label));
                            
                            // 添加跳转到循环头
                            preheader->instructions.push_back(
                                std::make_shared<GotoInstr>(
                                    std::make_shared<Operand>(OperandType::LABEL, loop.header->label)));
    ...
    for (auto& loop : detectedLoops) {
        ...
        for (int blockId : loop.blocks) {
            for (auto& block : blocks) {
                if (block->id == blockId) {
                    std::vector<std::shared_ptr<IRInstr>> toHoist;
                    ...
                    for (size_t i = 0; i < block->instructions.size(); ++i) {
                        ...
                        if (auto binOp = std::dynamic_pointer_cast<BinaryOpInstr>(instr)) {
                            ...
                            auto checkOperand = [&](const std::shared_ptr<Operand>& op) {
                                if (op->type == OperandType::VARIABLE || op->type == OperandType::TEMP) {
                                    if (loopDefs.find(op->name) != loopDefs.end()) {
                                        isInvariant = false;
                                    }
                                }
                            };
                            
                            checkOperand(binOp->left);
                            checkOperand(binOp->right);
                            
                            if (isInvariant) {
                                toHoist.push_back(instr);
                            ...
                            if (unaryOp->operand->type == OperandType::VARIABLE || 
                                ...
                                if (loopDefs.find(unaryOp->operand->name) != loopDefs.end()) {
                                    isInvariant = false;
                                }
                            }
                    ...
                    if (!toHoist.empty()) {
                        // 在preheader的跳转指令前插入
                        auto& preheaderInstrs = loop.preheader->instructions;
                        size_t insertPos = preheaderInstrs.size() - 1; // 在跳转前插入
                        
                        preheaderInstrs.insert(preheaderInstrs.begin() + insertPos,
                                             toHoist.begin(), toHoist.end());
 ...
 * 简化的函数内联优化
 */
void IRGenerator::collectFunctionInfo() {
    ...
    for (auto& instr : instructions) {
        if (auto call = std::dynamic_pointer_cast<CallInstr>(instr)) {
            functionCallCount[call->funcName]++;
        }
    }
    
    // 第二次遍历：收集函数体
    std::string currentFunc;
    std::vector<std::shared_ptr<IRInstr>> currentBody;
    bool inFunction = false;
    
    for (auto& instr : instructions) {
        if (auto funcBegin = std::dynamic_pointer_cast<FunctionBeginInstr>(instr)) {
            if (inFunction && !currentFunc.empty()) {
                // 保存前一个函数
                FunctionInfo info;
                info.body = currentBody;
                // 简化：不收集参数
                functionBodies[currentFunc] = info;
            }
            
            currentFunc = funcBegin->funcName;
            currentBody.clear();
            inFunction = true;
        }
        else if (auto funcEnd = std::dynamic_pointer_cast<FunctionEndInstr>(instr)) {
            if (inFunction && funcEnd->funcName == currentFunc) {
                // 保存函数
                FunctionInfo info;
                info.body = currentBody;
                functionBodies[currentFunc] = info;
        ...
        else if (inFunction && !currentFunc.empty()) {
            currentBody.push_back(instr);
        }
    }
}

bool IRGenerator::shouldInlineFunction(const std::string& funcName, int callCount) {
    // 不内联main函数
    if (funcName == "main") return false;
    
    auto it = functionBodies.find(funcName);
    if (it == functionBodies.end()) return false;
    
    const auto& info = it->second;
    
    // 简化规则：只内联很小的函数（少于10条指令）
    if (info.body.size() > 10) return false;
...
void IRGenerator::functionInlining() {
    ...
    for (size_t i = 0; i < instructions.size(); ++i) {
        ...
        if (auto call = std::dynamic_pointer_cast<CallInstr>(instr)) {
            // 检查是否应该内联
            int callCount = functionCallCount[call->funcName];
            
            if (shouldInlineFunction(call->funcName, callCount)) {
                auto it = functionBodies.find(call->funcName);
                if (it != functionBodies.end()) {
                    // 内联函数体
                    for (auto& bodyInstr : it->second.body) {
                        // 复制指令（简化：直接使用原指令）
                        newInstructions.push_back(bodyInstr);
                    }
                    continue;
                }
            }
    ...
    // 更新指令列表
    instructions.swap(newInstructions);
}
