...
/**
 * 循环不变量外提优化 (Loop Invariant Code Motion)
 * 将循环内不变的计算移到循环外执行。
 */
void IRGenerator::loopInvariantCodeMotion() {
    ...
    // BlockID -> BasicBlock映射
    std::unordered_map<int, BasicBlock> blocksMap;
    for (auto& b : blocks) blocksMap[b->id] = *b;
    ...
    for (auto& edge : backEdges) {
        ...
        // 获取循环内定义的变量
        auto loopDefs = getLoopDefs(loopBlocks, blocksMap);

        // 收集循环不变量指令
        std::vector<std::pair<std::shared_ptr<BasicBlock>, size_t>> invariants;

        for (auto blkId : loopBlocks) {
            auto it = blocksMap.find(blkId);
            if (it == blocksMap.end()) continue;
            ...
            for (size_t i = 0; i < block.instructions.size(); ++i) {
                ...
                // 检查是否是循环不变量
                if (isLoopInvariant(instr, loopDefs)) {
                    invariants.emplace_back(std::make_shared<BasicBlock>(block), i);
                }
            }
        }
        ...
        if (!invariants.empty()) {
            // 找到循环入口块（toBlk）
            auto loopEntryIt = std::find_if(blocks.begin(), blocks.end(),
                [toBlk](const std::shared_ptr<BasicBlock>& b) { return b->id == toBlk; });

            if (loopEntryIt != blocks.end()) {
                auto& loopEntry = *loopEntryIt;

                // 将不变量指令插入到循环入口前
                for (auto& inv : invariants) {
                    auto& srcBlock = inv.first;
                    size_t idx = inv.second;

                    // 复制指令
                    auto invariantInstr = srcBlock->instructions[idx];

                    // 插入到循环入口前
                    loopEntry->instructions.insert(loopEntry->instructions.begin(), invariantInstr);

                    // 从原位置移除
                    srcBlock->instructions.erase(srcBlock->instructions.begin() + idx);
                }
            }
        }
    }
 ...
 * 检查指令是否是循环不变量
 */
bool IRGenerator::isLoopInvariant(const std::shared_ptr<IRInstr>& instr,
    ...
    for (const auto& var : usedVars) {
        if (loopDefs.count(var)) {
            return false; // 在循环内被定义，不是不变量
        }
    }

    // 确保指令没有副作用（只考虑赋值和运算指令）
    if (std::dynamic_pointer_cast<AssignInstr>(instr) ||
        std::dynamic_pointer_cast<BinaryOpInstr>(instr) ||
        std::dynamic_pointer_cast<UnaryOpInstr>(instr)) {
        return true;
    }
...
/**
 * 函数内联优化 (Function Inlining)
 * 将函数调用替换为函数体的副本，以减少调用开销。
 */
void IRGenerator::functionInlining() {
    // 收集所有函数定义
    std::unordered_map<std::string, std::vector<std::shared_ptr<IRInstr>>> functionBodies;
    std::unordered_map<std::string, std::vector<std::string>> functionParams;

    // 第一遍：收集函数定义
    size_t i = 0;
    while (i < instructions.size()) {
        if (auto funcBegin = std::dynamic_pointer_cast<FunctionBeginInstr>(instructions[i])) {
            std::string funcName = funcBegin->funcName;
            std::vector<std::shared_ptr<IRInstr>> body;
            std::vector<std::string> params;

            // 收集参数
            i++;
            while (i < instructions.size()) {
                if (auto param = std::dynamic_pointer_cast<ParamInstr>(instructions[i])) {
                    params.push_back(param->param->name);
                    body.push_back(instructions[i]);
                    i++;
                } else {
                    break;
                }
            ...
            // 收集函数体直到FunctionEnd
            while (i < instructions.size()) {
                if (std::dynamic_pointer_cast<FunctionEndInstr>(instructions[i])) {
                    body.push_back(instructions[i]);
                    break;
                }
                body.push_back(instructions[i]);
    ...
    for (size_t idx = 0; idx < instructions.size(); ++idx) {
        ...
        if (auto call = std::dynamic_pointer_cast<CallInstr>(instr)) {
            std::string funcName = call->funcName;

            // 检查是否可以内联：函数存在、非递归、函数体不大
            if (functionBodies.count(funcName) &&
                shouldInlineFunction(funcName, functionBodies[funcName])) {

                // 获取函数体和参数
                auto& body = functionBodies[funcName];
                auto& params = functionParams[funcName];

                // 创建参数映射（调用实参 -> 函数形参）
                std::unordered_map<std::string, std::string> paramMap;
                size_t paramIdx = 0;

                // 查找前面的ParamInstr来获取实参
                for (int j = static_cast<int>(idx) - 1; j >= 0; --j) {
                    if (auto paramInstr = std::dynamic_pointer_cast<ParamInstr>(instructions[j])) {
                        if (paramIdx < params.size()) {
                            paramMap[params[paramIdx]] = paramInstr->param->name;
                            paramIdx++;
                        }
                    } else {
                        break; // 遇到非ParamInstr，停止
                    }
                }

                // 复制函数体，替换参数和变量
                std::vector<std::shared_ptr<IRInstr>> inlinedBody;
                std::unordered_map<std::string, std::string> varRenames;

                for (auto& bodyInstr : body) {
                    auto clonedInstr = cloneInstruction(bodyInstr, paramMap, varRenames);

                    // 处理返回值：将return替换为赋值给调用结果
                    if (auto ret = std::dynamic_pointer_cast<ReturnInstr>(clonedInstr)) {
                        if (call->result && ret->value) {
                            auto assign = std::make_shared<AssignInstr>(call->result, ret->value);
                            inlinedBody.push_back(assign);
                        }
                        // 添加跳转到函数结束（如果需要）
                    } else if (!std::dynamic_pointer_cast<FunctionBeginInstr>(clonedInstr) &&
                               !std::dynamic_pointer_cast<FunctionEndInstr>(clonedInstr) &&
                               !std::dynamic_pointer_cast<ParamInstr>(clonedInstr)) {
                        inlinedBody.push_back(clonedInstr);
                    }
                }

                // 移除前面的ParamInstr
                while (!newInstructions.empty()) {
                    if (std::dynamic_pointer_cast<ParamInstr>(newInstructions.back())) {
                ...
                // 添加内联的函数体
                newInstructions.insert(newInstructions.end(), inlinedBody.begin(), inlinedBody.end());
 ...
 * 判断函数是否应该内联
 */
bool IRGenerator::shouldInlineFunction(const std::string& funcName,
                                       const std::vector<std::shared_ptr<IRInstr>>& body) {
    // 简单的内联策略：函数体指令数少于阈值
    const size_t MAX_INLINE_SIZE = 20;

    size_t instrCount = 0;
    for (auto& instr : body) {
        if (!std::dynamic_pointer_cast<FunctionBeginInstr>(instr) &&
    ...
    return instrCount <= MAX_INLINE_SIZE;
}

/**
 * 克隆指令并替换变量名
 */
std::shared_ptr<IRInstr> IRGenerator::cloneInstruction(
    ...
    // 简单的深拷贝实现（实际需要更完整的拷贝）
    if (auto assign = std::dynamic_pointer_cast<AssignInstr>(instr)) {
        auto newTarget = renameOperand(assign->target, paramMap, varRenames);
        auto newSource = renameOperand(assign->source, paramMap, varRenames);
        return std::make_shared<AssignInstr>(newTarget, newSource);
    }
    else if (auto binOp = std::dynamic_pointer_cast<BinaryOpInstr>(instr)) {
        auto newResult = renameOperand(binOp->result, paramMap, varRenames);
        auto newLeft = renameOperand(binOp->left, paramMap, varRenames);
        auto newRight = renameOperand(binOp->right, paramMap, varRenames);
        return std::make_shared<BinaryOpInstr>(binOp->opcode, newResult, newLeft, newRight);
    }
    else if (auto unaryOp = std::dynamic_pointer_cast<UnaryOpInstr>(instr)) {
        auto newResult = renameOperand(unaryOp->result, paramMap, varRenames);
        auto newOperand = renameOperand(unaryOp->operand, paramMap, varRenames);
        return std::make_shared<UnaryOpInstr>(unaryOp->opcode, newResult, newOperand);
    }
    else if (auto ret = std::dynamic_pointer_cast<ReturnInstr>(instr)) {
        auto newValue = ret->value ? renameOperand(ret->value, paramMap, varRenames) : nullptr;
        return std::make_shared<ReturnInstr>(newValue);
    }
 ...
 * 重命名操作数
 */
std::shared_ptr<Operand> IRGenerator::renameOperand(
    ...
    if (operand->type == OperandType::VARIABLE || operand->type == OperandType::TEMP) {
        std::string newName = operand->name;

        // 首先检查是否是参数
        if (paramMap.count(operand->name)) {
            newName = paramMap.at(operand->name);
        } else {
            // 为局部变量创建新名称避免冲突
            if (varRenames.count(operand->name)) {
                newName = varRenames[operand->name];
            } else {
                newName = operand->name + "_inline_" + std::to_string(tempCount++);
                varRenames[operand->name] = newName;
            }
        }

        return std::make_shared<Operand>(OperandType(operand->type), newName);
    }
